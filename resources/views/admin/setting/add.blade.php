...
@section('sort-menu')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item"><a href="">Add Setting</a></li>
    </ol>
@endsection
@section('noidung')
    <div class="row float-right ">
        ...
        <div class="btn-group ">
            <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true"
                aria-expanded="false">
                {{ request()->type != null ? request()->type : $setting->type }}
            </button>
    ...
    <form action="{{ route('setting-create') . '?type=' . request()->type }}" method="post">
        ...
        <div class="form-group">
            ...
            <input id="key" class="form-control" placeholder="Mời nhập Key..." type="text" name="key">
        </div>
        <div class="form-group">
            <label for="value_key">Value Key</label>
            @if (request()->type === 'text')
                <input id="value_key" value="{{ old('value_key') }}" class="form-control" placeholder="Mời nhập Value..."
                    type="text" name="value_key">
            @else
                <textarea name="value_key" id="value_key" cols="30" class="form-control" rows="10"
                    placeholder="Mời nhập Value...">{{ old('value_key') }}</textarea>
            @endif
        </div>
