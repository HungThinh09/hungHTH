...
@section('noidung')
    ...
    <form action="{{ route('role-update') }}" method="post">
        ...
        <div class="form-group">
            ...
            <input id="name" class="form-control" type="text" value="{{ $role->name }}" placeholder="Nhập tên role..."
                name="name">
            <input class="form-control" type="hidden" name="id" value="{{ $role->id }}">
        </div>
        <div class="form-group">
            <label for="display">Display Name</label>
            <textarea name="display" class="form-control" id="display" cols="30"
                            ...
                            <div class="col-3">
                                <h5 class="card-title">
                                    <input type="checkbox" 
                                          ...
                                          {{$role->permisstion->contains('id',$chilPer->id)?'checked':''}}
                                        
                                    class="chilPer" name="chilPer[]" id=""
                                        value="{{ $chilPer->id }}">
                                    <label for="">
                                        {{ $chilPer->name }}
                                    </label>
